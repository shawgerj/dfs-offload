...
{
  // shawgerj: I'm leaving some of this list processing stuff in here in
  // case we want to handle multiple clients later on, but in general this
  // all needs to change... 
    int nodenum = 0, num_nodes=0;
    char node_name_prefix[128] = {'\0'};
    char *j = node_name_prefix;

    for (char *i = line; *i != 0; i++) {
        if ((*i >= '0') && (*i <= '9')) {
            nodenum = nodenum * 10 + *i - '0';
        } else {
            *j++ = *i;
        }
    }
    ...
    *node_list = (char **) calloc (num_nodes, sizeof(char *));
	...
	sprintf((*node_list)[k], "%s%d", node_name_prefix, nodenum);
    }
...
int get_rank ()
{
    ...
    // cloudlab machines have names like nodeX.some.other.junk
    // just keep the nodeX part
    int c = 0;
    while (c < 64 && (hostname[c] = utsname_buf.nodename[c]) != '.') {
      c++;
    }
    hostname[c] = '\0';
