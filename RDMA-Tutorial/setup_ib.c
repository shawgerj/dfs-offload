...
int setup_ib ()
{
	...
	.srq     = ib_res.srq,
        .cap = {
	  // max_send_wr and max_recv_wr should be ignored if an SRQ
	  // is set, but apparently setting these two fields to max
	  // causes ibv_create_qp to fail with "Invalid Argument"
	  // more info here if needed
	  // https://linux-rdma.vger.kernel.narkive.com/rkR0gUjT/rdma-create-qp-and-max-send-wr
